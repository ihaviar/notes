...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyCTJw9vwEFLBgvonaxgISvGfy6VX8sAwcU'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $directionsStart1			= "";						// The starting location (lat/long co-ordinate or address) of the directions. Set to 'auto' to default it to the users location
	var $directionsEnd1				= "";
	...
	var $placesAutocompleteInputID2	= '';	
	...
	var $placesAutocompleteBoundsMap= TRUE;					// An alternative to setting the SW and NE bounds is to use the bounds of the current viewport. If set to TRUE, the bounds will be set to the viewport of the visible map, even if dragged or zoomed
	...
	function create_map()
	{
		...
		if ($this->places) {
			...
			if ($this->placesAutocompleteInputID != "")
			{
				...
				var autocompleteInput2 = document.getElementById(\''.$this->placesAutocompleteInputID2.'\');
				...
				if ($this->placesAutocompleteBoundsMap)
				{
					...
					'.'placesAutocomplete2.bindTo(\'bounds\', map);									
					';	//nastavenie bounds pre obe autocomplete
				}
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		$data_location .= "maps.google.com/maps/api/geocode/json?address=".urlencode(utf8_encode($address))."&key=".$this->apiKey;
